...
class TransCNN(nn.Module):
    ...
    def __init__(
        ...
        self.ln2 = nn.LayerNorm(in_channels * 2)
        ...
        self.mlp = nn.Sequential(
            nn.Conv1d(in_channels * 2, n_hidden, 1),
            act_layer(),
        ...
        if path_pdrop > 0.0:
            ...
            self.drop_path_conv = AffineDropPath(in_channels, drop_prob=path_pdrop)
        else:
            self.drop_path_attn = nn.Identity()
            self.drop_path_mlp = nn.Identity()
            self.drop_path_conv = nn.Identity()

        self.psi = nn.Conv1d(in_channels, in_channels, 3, stride=1, padding=3 // 2, groups=in_channels)
        self.fc = nn.Conv1d(in_channels, in_channels, 1, stride=1, padding=0, groups=in_channels)
        self.convw = nn.Conv1d(in_channels, in_channels, 3, stride=1, padding=3 // 2, groups=in_channels)
        self.convkw = nn.Conv1d(in_channels, in_channels, 5, stride=1, padding=5 // 2, groups=in_channels)
        self.global_fc = nn.Conv1d(in_channels, in_channels, 1, stride=1, padding=0, groups=in_channels)
        self.reset_params(init_conv_vars=1)
        
    def reset_params(self, init_conv_vars=1):
        # 使用 Kaiming 初始化卷积权重
        torch.nn.init.kaiming_normal_(self.psi.weight, mode='fan_out', nonlinearity='relu')
        torch.nn.init.kaiming_normal_(self.fc.weight, mode='fan_out', nonlinearity='relu')
        torch.nn.init.kaiming_normal_(self.convw.weight, mode='fan_out', nonlinearity='relu')
        torch.nn.init.kaiming_normal_(self.convkw.weight, mode='fan_out', nonlinearity='relu')
        torch.nn.init.kaiming_normal_(self.global_fc.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, x, mask):
        ...
        x_conv = x
        # transformer
        out_trans, out_mask = self.attn(x, mask)
        out_mask_float = out_mask.to(out_trans.dtype)
        out = self.pool_skip(x) * out_mask_float.unsqueeze(1) + self.drop_path_attn(out_trans)

        # CNN
        psi = self.psi(x_conv)
        fc = self.fc(x_conv)
        convw = self.convw(x_conv)
        convkw = self.convkw(x_conv)
        phi = torch.relu(self.global_fc(x_conv.mean(dim=-1, keepdim=True)))
        out_conv = fc * phi + (convw + convkw) * psi + x_conv
        out_conv = self.pool_skip(out_conv) * out_mask_float.unsqueeze(1) + self.drop_path_conv(out_conv)

        out_sum = torch.concat([out, out_conv], dim=1)
        # FFN
        out = out + self.drop_path_mlp(
            self.mlp(self.ln2(out_sum.permute(0, 2, 1)).permute(0, 2, 1)) * out_mask_float.unsqueeze(1)
        )
        return out, out_mask
